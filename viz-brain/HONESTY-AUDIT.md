...
spike at full strength for 12 **rendered frames**. At the measured 28–29 fps that hold is only
...
|---|---|---|---|---|---|
| A (fps 18) | 15–42 | 6 | 2.91% | 9.81 | true |
...
`warm` 2…274, `hot` 6…64, `max_r_minus_b` 167–171, **never 0**, with `spikeCount` 2–3 at every
sample (a separate 48-sample telemetry poll: `spikeCount` 2–3 continuously over 12 s).
...
|---|---|---|---|
| 0.055 | 12.77% | — | 2.23 |
| 0.012 (shipped) | 2.91% | (10, 15, 20) | 9.81–9.83 |
...
2–3% target and comes from the ambient/anatomy term; live is untouched (`uQuiet` is 1.0 on a
live frame, `uAmbient` stays 0.1, `uGate` is live/dead only). The dead control's motion is
exactly 0 changed pixels between consecutive captures, versus ~19,000 (1.8–2.0%) live.
...
`trail_moving` now reads true: `moved_pct` is 1.8–2.0% between consecutive live captures and
