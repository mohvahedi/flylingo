...
- **A public reproducible Colab notebook. DONE.** `colab/FlyLingo.ipynb`, generated by
  ...
  - The original blocker, for the record: the parent repo has **NO GIT REMOTE**, so a notebook could
    not clone it. Embedding solved it without publishing anything.
  - The data still needs no upload: the feathers are in a **public GCS bucket**, and `step1_build.py`
    downloads and digest-verifies them (`connectome-weights-…-minconf-0.5.feather` = 1,051,241,946
...
- **The viral post / video. STILL NOT SHIPPABLE, two reasons, both fixable.**
  1. The 45 s capture at `C:/Users/Lion/Desktop/FlyLingo-45s.mp4` was recorded **before** the
     ...
     behind. **Re-record before publishing** (`tools/record_demo.py`).
...
**Still genuinely open:** wherever the code is meant to be published, there is no git remote yet
(§2), and `brain/runs/plastic_brain/` holds 9.7 MB of run artifacts that are untracked and should
